<script lang="ts">
  // copied from https://github.com/skeletonlabs/skeleton/blob/master/packages/skeleton/src/lib/components/Paginator/Paginator.svelte
  // in order to allow 0 based paging
	import { createEventDispatcher } from 'svelte';
	// Types
    // Types
  import type { PaginationSettings, CssClasses } from '@skeletonlabs/skeleton';
      ...
      -->
		...
		{#if showNumerals === false}
			...
			<button type="button" class="{buttonClasses} pointer-events-none !text-sm">
				{settings.page * settings.limit}-{Math.min(settings.page * settings.limit + settings.limit - 1, settings.size - 1)}&nbsp;<span
